...
namespace EasySII.Business
{
    ...
    /// Factura emitida.
    /// </summary>
    public class ARInvoiceAV : Invoice
    {
        ...
        public ARInvoiceAV(RegistroLRFacturasEmitidas registroLRFacturasEmitidas)
        {
            ...
            if (!string.IsNullOrWhiteSpace(siiInvoice.FacturaExpedida.FechaOperacion))
            ...
            Sujeta sujeta = null;
            NoSujeta noSujeta = null;

            if (siiInvoice.FacturaExpedida.TipoDesglose.DesgloseFactura != null)
            {
                if (siiInvoice.FacturaExpedida.TipoDesglose.DesgloseFactura.Sujeta != null)
                {
                    sujeta = siiInvoice.FacturaExpedida.TipoDesglose.DesgloseFactura.Sujeta;
                }

                if (siiInvoice.FacturaExpedida.TipoDesglose.DesgloseFactura.NoSujeta != null)
                {
                    noSujeta = siiInvoice.FacturaExpedida.TipoDesglose.DesgloseFactura.NoSujeta;
                }
            }
            else
            {
            ...
            if (noSujeta != null)
            {
                ...
                if (noSujeta.ImportePorArticulos7_14_Otros != null)
                {
                    IsTaxExcluded = true;
                    GrossAmount = Convert.ToDecimal(noSujeta.ImportePorArticulos7_14_Otros, Settings.DefaultNumberFormatInfo);
                }

                if (noSujeta.ImporteTAIReglasLocalizacion != null)
                {
                    IsTAIRules = true;
                    GrossAmount = Convert.ToDecimal(noSujeta.ImporteTAIReglasLocalizacion, Settings.DefaultNumberFormatInfo);
                }
            }
		...
		public RegistroLRFacturasEmitidas ToSII(bool updateInnerSII = false, bool skipErrors = false)
        {
                ...
                ClaveRegimenEspecialOTrascendencia == ClaveRegimenEspecialOTrascendencia.ArrendamientoLocalNegocioAmbosImportacionesNoDua)
            {
                ...
                if(BuildingState == BuildingState.SinValor)
                    throw new ArgumentNullException($"For ClaveRegimenEspecialOTrascendencia=" +
                        $"{ClaveRegimenEspecialOTrascendencia} BuildingState is required.");

                if (string.IsNullOrEmpty(BuildingReference))
                    throw new ArgumentNullException($"For ClaveRegimenEspecialOTrascendencia=" +
                        $"{ClaveRegimenEspecialOTrascendencia} BuildingReference is required.");
            ...
            if (TaxesOutputs.Count > 0)
            {
				...
				foreach (KeyValuePair<decimal, decimal[]> taxOut in TaxesOutputs)
				{
					...
					else if (taxOut.Key == 0)
					{
                        ...
                        if (!string.IsNullOrWhiteSpace(TipoSujeta))
                        {
                            // Operación no exenta (Se puede dar el caso de que se trate de un IVA Incluido, el cual no está desglosado y el Tipo Impositivo es cero, pero se tiene que
                            //                      informar 'desglosado')
                            if (sujeta.NoExenta == null)
                            {
